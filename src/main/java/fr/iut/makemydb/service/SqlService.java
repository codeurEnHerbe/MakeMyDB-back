...
public class SqlService {
    ...
    private String createRelations(SchemaInfos infos){
        ...
        infos.getRelations().forEach( relation -> {
            ...
            if( link2.getCardinalMax().equals("1") && link1.getCardinalMax().equals("1") ){
                ArrayList<Attribute> newAttributes = e1.getElement().getAttributes();
                newAttributes.addAll(e2.getElement().getAttributes());
                sqlEentities.remove(e1.getElement());
                sqlEentities.remove(e2.getElement());
                sqlEentities.add(new Element(relation.getElement().getName(), newAttributes));
                System.out.print(sqlEentities);
            }
            if( link2.getCardinalMax().equals("n") && !link1.getCardinalMax().equals("n") ){
                e1.getElement().getAttributes().add(
                        new Attribute("fk_" + e2.findAttributePrimary().getName(),
                                e2.findAttributePrimary(), e2));
            }else if( link1.getCardinalMax().equals("n") && !link2.getCardinalMax().equals("n") ){
                e2.getElement()
                        .getAttributes()
                        .add(new Attribute("fk_" +
                                e1.findAttributePrimary().getName(),
    ...
    private String createAttributes(ArrayList<Attribute> attributes){
        ...
        attributes.forEach(attribute -> {
            ...
            if(attribute.getTypeNumber() != 0){
                stringBuild.append("(" + attribute.getTypeNumber() + ")");
            }
        });
        stringBuild.append("," + "\n\t\t" + "PRIMARY KEY (");
